...
def show_form_window(date,serial,guest,temp):#日期,序號,廠商名,資料
    ...
    button_fix.place(x=idicent*1,y=window.winfo_width()-30)
    button_save = tk.Button(window, text = "儲存表單", command = onsave,bg='yellow')
    button_save.place(x=idicent*2,y=window.winfo_width()-30)
    button_fix_save = tk.Button(window, text = "修改成本", command = onfixsave,bg='gray')
    button_fix_save.place(x=idicent*3,y=window.winfo_width()-30)

    window.mainloop()

def show_to_window(temp,choose):#來源資料,(1 客戶名稱 2 產品用 3 產品輸入欄),
            temp=temp#顯示產品輸入
            choose=choose
            window = tk.Tk()
            if choose==1:
                    ...
                    if keyin==3:#結束區
                        ...
                        for i in range(2,goods_a+1):
                            ...
                            if(new_name in d2[0]  ):
                                lable=str(i)
                                list_inquire.append(d2)
                        temp_goods=list_inquire[0]
                        print("old",temp_goods)#[1]成本 [2]大賣  [3]小賣
                        print("產品名稱 : ",temp_goods[0])
                        
                        print("修改成本","(",temp_goods[1],")  :",end="")
                        keyin=(input())
                        #print(type(keyin))
                        while keyin == "":
                            print("修改成本","(",temp_goods[1],")  :",end="")
                        ...
                        keyin=float(keyin)
                        temp_goods[3]=keyin
                        print("new ",temp_goods)
                        #print(lable)
                        #print(path_goods)
                        #print("now now top",temp_goods)
                        print("確定要存檔嗎  1: 是  2: 不用存檔  3: 存檔且結束表單 ",end="")
                        number=(input())
                        number=check_enter(number)
                        if number==1 or  number==3:
                            temp_goods=json.dumps(temp_goods)####???
                            
                            
                            #print(path_goods)

                            #print(lable)
                            #print("now now",temp_goods)
                            update(path_goods,lable,temp_goods)##建立 位置/檔名/資料/
                            date_temp=(readdate(path_goods))#產品與電線用
                            print("update")
                            if number==3:
                                newname=serial+head[1]
