...
def show_allform_window():#日期,序號,廠商名,資料
    ...
    def find_goods(keyin,flag):#找到 產品  資料  不顯示
        ...
        if(len(list_inquire)==0):
            ...
            middle_clean()
            middle_show_b(1,"產品資訊",dict_goods)#01-01-02
    ...
    #0010 1 只顯示不修改  2 修改 #01-01 #01-02
    ...
    #0014
    def data_save_or_delete(mode,temp):#01-02-04
        ...
        if mode==1:
            ...
            #main_read()
            #date_temp=(readdate(path_goods))#產品與電線用####use
        #print(temp)   
        #print(lable)   
        #print(path_goods)    
    ...
    #0002a 顯示選項
    def middle_show_a(mode,title,temp):#模式 0 不顯 1 只顯示產品,表頭,資料
    ...
    #0002b
    def middle_show_b(mode,title,temp):#模式 0 不顯 1 只顯示產品,表頭,資料
        list1 = ["名稱","成本","大賣","小賣","編號","單位","數量","簡稱"]
        ...
        if mode==1:
            ...
            def Selection ():
                    ...
                    number=(temp.get(var.get()))
                    print(number[1])
                    test_show_1(number[1])#01-01
                    find_goods_lable(number[1],1)
    ...
    def main_read():
        ...
        print("in main net==",keep_for_net)
        keep_for_program=int(date_temp.get('keep'))#1229use
        print("in main program==",keep_for_program)
