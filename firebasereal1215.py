...
def show_form_window(date,serial,guest,temp):#日期,序號,廠商名,資料
    ...
    #button_next = tk.Button(window, text = "next", command = onOK)
    #button_next.place(x=600,y=40)
        ...
        while choose_a != 88:
            ...
            if choose_a==2:
                ...
                while next != 0:
                    ...
                    keep_temp=[]
                    keyin=(input("1: 下一筆資料  2:修改資料   3: 總計(結束表單) 4: 修改產品資料      :  "))
                    
                    while keyin.isdigit()!=True :
                            print("\n 1: 下一筆資料  2:修改資料   3: 總計(結束表單) 4: 修改產品資料      :  ",end='')
                            keyin=(input())
                    keyin=int(keyin)
                    while keyin > 4:
                            print("\n 1: 下一筆資料  2:修改資料   3: 總計(結束表單) 4: 修改產品資料      :  ",end='')
                    ...
                    if keyin==3:#結束區
                        ...
                        #print(lable)
                        #print(path_goods)
                        print("確定要存檔嗎  1: 是  2: 不用存檔  3: 存檔且儲存表單 ",end="")
                        number=(input())
                        number=check_enter(number)
